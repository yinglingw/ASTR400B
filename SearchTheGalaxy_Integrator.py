...
def DynamicalFriction(Msat, x, y, z, vx, vy, vz, dummy):
    ...
    v_circ = circular_speed_hern(Msat, 62., r)
    #v_circ = 250.
    b_max = r
    b_min = BIG_G * Msat / v_circ**2  # 250 is circ speed
...
def OrbitIntegrator(t_init, dt, t_final, file):
    ...
    if file == 1:
        ...
        open_future_M33 = open("Future_M33_Post_Merge.txt", '+w')
...
def circular_speed_hern(mass, scale_length, radius):
    ...
    v_circ_at_r = np.sqrt(BIG_G * mass / (radius + scale_length)**2 * radius)
...
def main():
    ...
    COM_file = ["COM_M31.txt", "COM_M33.txt"]
    ...
    OrbitIntegrator(6.48571, 0.01, 14., 1)

    # open and read in the values for idealized scenario
    integration_file = "Future_M33_Opt1.txt"
    time, xx, yy, zz, vxx, vyy, vzz = read_COM(integration_file)

    # open and read in values for scenario based on Van der Marel and Beslas 2012 simulation
    integration_file2 = "Future_M33_Post_Merge.txt"
    ...
    # an array with appropriate intervals for the sim
    sim_time = np.arange(0, 10., 0.014268)
    #sim_time = np.arange(0, 11.4143, 0.014268)

    # calc values for M33 wrt to M31
    ...
    # plot position of M33 wrt to M31
    line25, = plt.plot(sim_time, r_sim, label="M33 Belsa Sim")
    ...
    legend = plt.legend(handles=[line25, line35], loc=5)
    ...
    plt.savefig("Integrator_Position_700.png")
    plt.show()

    # plot velocity of M33 wrt to M31
    line24, = plt.plot(sim_time, v_sim, label="M33 Besla Sim")
    line34, = plt.plot(time2, total_velocity, label="M33 Integrator")
    # line44, = plt.plot(time2, total_velocity_circ, label="M33 Vel Circ")
    legend = plt.legend(handles=[line24, line34], loc="upper right")
    plt.title("M33 Vel vs time")
    plt.xlabel("time in Myr")
    plt.ylabel("velocity in km/s")
    # plt.xlim(0,10000)
    plt.ylim(0, 1000)
    plt.savefig("Integrator_Velocity_700.png")
    plt.show()
