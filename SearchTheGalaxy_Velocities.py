...
def main():
    ...
    open_stdev = open("Stdev_Histos.txt", '+w')

    open_stdev.write("diff f b")
    open_stdev.write("\n")
    ...
    while snap < 700:
        ...
        for i in range(len(x_local)):
            dot_prod_local = np.dot([vx_COM_M33[snap], vy_COM_M33[snap], vz_COM_M33[snap]],[x_local[i], y_local[i], z_local[i]])

            #print(dot_prod_local)
            dot_prod_M31 = np.dot([x_to_M31, y_to_M31, z_to_M31] ,[x_local[i], y_local[i], z_local[i]])

            if dot_prod_local < 0. and dot_prod_M31 > -5. and dot_prod_M31 < 5.:
                x_back_particles.append(x_local[i])
                vx_back_particles.append(vx_local[i])
                y_back_particles.append(y_local[i])
                vy_back_particles.append(vy_local[i])
                z_back_particles.append(z_local[i])
                vz_back_particles.append(vz_local[i])
            elif dot_prod_local > 0. and dot_prod_M31 > -5. and dot_prod_M31 < 5.:
                x_front_particles.append(x_local[i])
                vx_front_particles.append(vx_local[i])
                y_front_particles.append(y_local[i])
                vy_front_particles.append(vy_local[i])
                z_front_particles.append(z_local[i])
                vz_front_particles.append(vz_local[i])

        print(len(x_back_particles), len(x_front_particles))
        ...
        diff_stdev = stdev_front - stdev_back
        ...
        # COMP = [x, y, z, vx, vy, vz]
        COMP = [diff_stdev, len(x_front_particles), len(x_back_particles)]

        # converting COMP to a string
        COMP_to_str = " ".join(str(j) for j in COMP)
        open_stdev.write(COMP_to_str)
        open_stdev.write("\n")


        #print("num of particles enclosed ", len(x_local))
        """
        rho_test = ds.grid(y_local_M33, z_local_M33, x_local_M33, 100)
        ...
        plt.ylabel("Z (kpc)")
        plt.title("Density of Halo Particles at Snap" + str(snap) + "  :" + str(len(y_local_M33)) + " Particles")
        ...
        file_name = "Wake_YZ_M33_700_Snap_" + str(snap) + ".png"
